...
! function(e) {
    ...
    U.dateRangePickerLanguages = {
        ...
        function C() {
            // 이미 선택된 날짜가 있는지 확인하고 있다면 해당 클래스 제거
            var $selectedDays = m.find(".day.checked");
            if ($selectedDays.length > 0) {
                $selectedDays.removeClass("checked first-date-selected last-date-selected");
            }

            if (f.start || f.end) {
                var start = Q(f.start).startOf("day").valueOf();
                var end = Q(f.end).startOf("day").valueOf();
                ...
                m.find(".day").each(function() {
                    ...
                    if ((f.start && f.end && time >= start && time <= end) || 
                        (f.start && !f.end && Q(f.start).format("YYYY-MM-DD") == Q(time).format("YYYY-MM-DD"))) {
                        $this.addClass("checked");
                        if (Q(time).format("YYYY-MM-DD") == Q(f.start).format("YYYY-MM-DD")) {
                            $this.addClass("first-date-selected");
                        } else if (Q(time).format("YYYY-MM-DD") == Q(f.end).format("YYYY-MM-DD")) {
                            $this.addClass("last-date-selected");
                        }
                    }
                });

                // 선택된 날짜 이후의 날짜를 클릭할 때 first-date-selected 클래스 제거
                m.find(".day").on("click", function() {
                    var clickedTime = parseInt(U(this).attr("time"));
                    if (Q(clickedTime).isAfter(Q(f.start), 'day')) {
                        m.find(".day.first-date-selected").removeClass("first-date-selected checked");
                    }
                });


                m.find(".week-number").each(function() {
                    if (U(this).attr("data-start-time") == f.startWeek) {
...
//        function k(e) {
//            var selectedDate = Q(e.data("time")).toDate();
//
//            // 현재 선택한 날짜와 시작일, 종료일이 다른 경우
//            if (!e.hasClass("first-date-selected") && !e.hasClass("last-date-selected")) {
//                // 현재 설정된 시작일과 종료일 클래스를 모두 제거
...
//
//                // 새로운 시작일로 설정하고 클래스 추가
//                e.addClass("first-date-selected");
//            } else if (e.hasClass("first-date-selected")) {
//                // 이미 시작일로 설정된 경우, 종료일로 설정하고 클래스 추가
//                e.removeClass("first-date-selected");
//                e.addClass("last-date-selected");
//            } else {
//                // 이미 종료일로 설정된 경우, 종료일 클래스 제거
//                e.removeClass("last-date-selected");
//            }
//
//            // 다음 작업 실행...
//        }
